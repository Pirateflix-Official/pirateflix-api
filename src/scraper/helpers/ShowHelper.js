...
export default class ShowHelper extends AbstractHelper {
  ...
  _getTmdbImages(tmdbId: number): Object {
    return tmdb.tv.images({
      ...
      const baseUrl = 'https://pirateflix.app      const tmdbPoster = i.posters.filter(
  ...
  _getTvdbImages(tvdbId: number): Object {
    return tvdb.getSeriesById(tvdbId).then(i => {
      const baseUrl = 'https://pirateflix.app      const { Holder } = AbstractHelper
      const images = {
   ...
   * @override
   * @param {!string} id - The slug to query https://pirateflix.app   * @returns {Show} - A new show without the episodes attached.
   */
